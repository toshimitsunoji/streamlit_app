...
<style>
    ...
    .forecast-box { background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; padding: 10px 2px; text-align: center; margin-bottom: 15px; box-shadow: 0 2px 5px rgba(0,0,0,0.02); }
</style>
...
# ==========================================
# 🎯 B. AI思考予報エンジン (LightGBMベース)
# ==========================================
def create_hourly_dataset(df_1min, df_sched_raw):
    """
    過去データから、LightGBM学習用の1時間ブロック単位のデータセットを作成する
    """
    df = df_1min.copy()
    ...
    for name, group in df.groupby('date_hour'):
        if not (9 <= name.hour <= 18): continue
        if group.empty: continue
        
        # ターゲット (深思考成功フラグ)
        cond_A = group['has_5m_focus'].max() > 0
        cond_B = group['fatigue_smooth'].mean() <= fatigue_th
        cond_C = group['low_arousal'].mean() <= la_th
        is_success = 1 if (cond_A and cond_B and cond_C) else 0
        
        # 直前の状態 (予測開始時のコンディション)
        prev_10m = df[(df.index < name) & (df.index >= name - pd.Timedelta(minutes=10))]
        fatigue_at_start = prev_10m['fatigue_smooth'].mean() if not prev_10m.empty else df['fatigue_smooth'].median()
        la_at_start = prev_10m['low_arousal'].mean() if not prev_10m.empty else 0
        
        # スケジュール空き時間
        free_mins = 60
        if df_sched_raw is not None and not df_sched_raw.empty:
            h_end = name + pd.Timedelta(hours=1)
            conflicts = df_sched_raw[(df_sched_raw['end_dt'] > name) & (df_sched_raw['start_dt'] < h_end)]
            conflict_mins = 0
            for _, row in conflicts.iterrows():
                c_start = max(name, row['start_dt'])
                c_end = min(h_end, row['end_dt'])
                conflict_mins += (c_end - c_start).total_seconds() / 60
            free_mins = max(0, 60 - conflict_mins)
            
        records.append({
            'datetime': name,
            'hour': name.hour,
            'dayofweek': name.dayofweek,
            'fatigue_at_start': fatigue_at_start,
            'la_at_start': la_at_start,
            'free_mins': free_mins,
            'target': is_success
        })
...
def train_lgbm_forecast_model(df_hourly):
    """
    作成したデータセットでLightGBMモデルを学習する
    """
    if df_hourly.empty or df_hourly['target'].nunique() <= 1:
        return None, []
        
    features = ['hour', 'dayofweek', 'fatigue_at_start', 'la_at_start', 'free_mins']
...
def predict_today_forecast(df_1min, df_sched_raw, model, features, t_now):
    """
    当日9〜18時の特徴量を生成し、AIモデルで成功確率と相対ランクを予測する
    """
    if model is None:
        return {h: '○' for h in range(9, 19)}, pd.Series({h: 0.5 for h in range(9, 19)}), pd.DataFrame()
        
    today_date = t_now.date()
    df_today = df_1min[df_1min.index.date == today_date]
    ...
    # 疲労ペース（傾き）の計算
    morning_df = df_today[(df_today.index.hour >= 9) & (df_today.index.hour <= 10)]
    fat_morning = morning_df['fatigue_smooth'].iloc[0] if not morning_df.empty else fat_now
    la_morning = morning_df['low_arousal'].iloc[0] if not morning_df.empty else la_now
    
    elapsed_hours = max(1.0, t_now.hour - 9 + (t_now.minute / 60.0))
    fat_slope = (fat_now - fat_morning) / elapsed_hours
    la_slope = (la_now - la_morning) / elapsed_hours
    ...
    for h in range(9, 19):
        ...
        if df_sched_raw is not None and not df_sched_raw.empty:
            ...
            conflict_mins = 0
            for _, row in conflicts.iterrows():
                c_start = max(h_start, row['start_dt'])
                c_end = min(h_end, row['end_dt'])
                conflict_mins += (c_end - c_start).total_seconds() / 60
            free_mins = max(0, 60 - conflict_mins)
            
        # 状態予測 (過去は実績、未来はトレンド補正)
        if h <= t_now.hour:
            prev_10m = df_1min[(df_1min.index < h_start) & (df_1min.index >= h_start - pd.Timedelta(minutes=10))]
            fat_at_start = prev_10m['fatigue_smooth'].mean() if not prev_10m.empty else fat_now
            la_at_start = prev_10m['low_arousal'].mean() if not prev_10m.empty else 0
        else:
            diff_hours = h - (t_now.hour + t_now.minute/60.0)
            fat_at_start = fat_now + fat_slope * diff_hours * 0.5 # 傾きを0.5倍に減衰
            la_at_start = la_now + la_slope * diff_hours * 0.5
            
        records.append({
            'hour': h,
            'dayofweek': h_start.dayofweek,
            'fatigue_at_start': fat_at_start,
            'la_at_start': la_at_start,
            'free_mins': free_mins
        })
        
    df_pred = pd.DataFrame(records)
    preds = model.predict_proba(df_pred[features])[:, 1]
    
    s_series = pd.Series(preds, index=df_pred['hour'])
    
    # 完全に空きがない枠は強制除外 (-1.0)
    s_series = s_series.where(df_pred.set_index('hour')['free_mins'] >= 15, -1.0)
    
    # AI予測確率に基づく相対評価 (上位20%, 20-50%, 50-80%, 下位20%)
    ranks = s_series.rank(method='first', ascending=False)
    tiers = {}
    for h, r in ranks.items():
        if s_series[h] == -1.0: tiers[h] = '×'
        elif r <= 2: tiers[h] = '◎'
        elif r <= 5: tiers[h] = '○'
        elif r <= 8: tiers[h] = '△'
        else: tiers[h] = '×'
        
    return tiers, s_series, df_pred
...
def recommend_today_deep_block(df_1min, df_sched_raw, today_scores_s, t_now):
    """
    AIの予測スコア(today_scores_s)とリアルタイム補正から、今日の勝負時間を1枠だけ提案
    """
    ...
    for b in blocks:
        ...
        h = start_dt.hour
        ai_prob = today_scores_s.get(h, 0)
        if ai_prob == -1.0: continue # 予定で埋まっている時間はスキップ
        
        # 厳密な直前会議ペナルティ (AIとは別で加味)
        penalty = 0
        ...
        score = ai_prob + penalty
        
        if score > best_score:
            best_score = score
            base_reasons = []
            base_reasons.append("AIが本日の中で最も成功確率が高いと予測した時間帯です。")
...
if file_ts is not None:
    with st.spinner("AIが深思考予測モデルを学習・推論中..."):
        # 1. データロード
        ...
        # 3. AI思考予報エンジン (LightGBM)
        df_hourly = create_hourly_dataset(df_1min, df_sched_raw)
        model, lgb_features = train_lgbm_forecast_model(df_hourly)
        today_tiers, today_scores_s, df_pred_today = predict_today_forecast(df_1min, df_sched_raw, model, lgb_features, t_now)
        best_deep_block, deep_reasons = recommend_today_deep_block(df_1min, df_sched_raw, today_scores_s, t_now)

        # 4. 波解析・コンディションエンジン (リアルタイム用)
        ...
        if la_band == '高' and fatigue_band == '高': action_text = "疲労と眠気がピークに達しています。無理な作業は控え、完全な休息を取ることを強く推奨します。"
    ...
    with col_h2:
        if st.button("🔄 最新状態でAI予測を更新", use_container_width=True):
            st.rerun()
    ...
    with tab_today:
        ...
        if best_deep_block:
            ...
            # AI SHAP 要因分析の表示
            if model is not None and not df_pred_today.empty:
                best_h = best_deep_block['adj_start_dt'].hour
                target_row = df_pred_today[df_pred_today['hour'] == best_h]
                if not target_row.empty:
                    with st.expander("🧠 なぜAIはこの時間を推薦したのか？ (要因分析)"):
                        st.write("LightGBMモデルがこの時間帯の成功確率を高く評価した理由（SHAP値）です。赤がプラス要因、青がマイナス要因を示します。")
                        explainer = shap.TreeExplainer(model)
                        shap_values = explainer(target_row[lgb_features])
                        fig_shap, ax_shap = plt.subplots(figsize=(8, 4))
                        # 2値分類の場合、クラス1(成功)のSHAP値をプロット
        ...
        st.markdown("### 🌤 今日のAI思考予報 (時間帯別 相対適性)")
        st.write("今日1日の中での「深思考への適正」をAIが予測し、相対評価（◎・○・△・×）で表しています。")
        ...
        for i, h in enumerate(range(9, 19)):
            ...
            with cols[i]:
                ...
                <div class="forecast-box" style="border-top: 4px solid {color};">
                    <div style="font-size: 0.9rem; font-weight: bold; color: #475569;">{h}:00</div>
                    <div style="font-size: 1.8rem; font-weight: 900; color: {color}; margin-top: 5px;">{tier}</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with col_s1:
            ...
            <div style="display: flex; gap: 10px;">
                <div class="kpi-card" style="flex: 1; padding: 15px;">
                    <div class="kpi-title" style="font-size:0.85rem;">今日の空白時間 集中率</div>
                ...
                <div class="kpi-card" style="flex: 1; padding: 15px;">
                    <div class="kpi-title" style="font-size:0.85rem;">今日の分断ロス(波の頓挫)</div>
